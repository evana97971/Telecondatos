...
class TeleenfermiaApp {
  ...
  setupEventListeners() {
    ...
    // Limpiar formulario al abrir modal de paciente
    document.getElementById('modalPaciente').addEventListener('show.bs.modal', () => {
      if (!this.editandoPacienteId) {
        document.getElementById('formPaciente').reset();
      }
    });

    // Limpiar formulario al abrir modal de consulta
    document.getElementById('modalConsulta').addEventListener('show.bs.modal', () => {
      document.getElementById('formConsulta').reset();
      document.getElementById('conFecha').valueAsDate = new Date();
    });
  }
  ...
  guardarPaciente() {
    ...
    const telefono = document.getElementById('pacTelefono').value.trim();
    const edad = parseInt(document.getElementById('pacEdad').value);

    // Validaciones
    if (!nombre || !dni || !email || !telefono || !edad) {
      alert('⚠️ Por favor completa todos los campos obligatorios.');
      return;
    }

    // Si es nuevo, validar DNI único
    if (!this.editandoPacienteId && this.pacientes.some(p => p.dni === dni)) {
      alert('⚠️ Este DNI ya está registrado.');
      return;
    }

    const paciente = {
      id: this.editandoPacienteId || Date.now(),
      nombre,
      dni,
      email,
      telefono,
    ...
    // Si es edición
    if (this.editandoPacienteId) {
      const index = this.pacientes.findIndex(p => p.id === this.editandoPacienteId);
      if (index !== -1) {
  ...
  mostrarPacientes() {
    ...
    tbody.innerHTML = filtrados.map(p => `
      <tr>
        ...
        <td>${p.telefono}</td>
        <td>
          <span class="badge ${p.estado === 'critico' ? 'bg-danger' : p.estado === 'activo' ? 'bg-success' : 'bg-secondary'}">
            ${p.estado}
          </span>
        </td>
        <td>
          <button class="btn btn-sm btn-info" onclick="window.app.editarPaciente(${p.id})" title="Editar">
            <i class="bi bi-pencil"></i>
          </button>
          <button class="btn btn-sm btn-danger" onclick="window.app.eliminarPaciente(${p.id})" title="Eliminar">
  ...
  // ==================== CONSULTAS ====================
  guardarConsulta() {
    const dni = document.getElementById('conDNI').value.trim();
    const tipo = document.getElementById('conTipo').value;
    const motivo = document.getElementById('conMotivo').value.trim();
  ...
  mostrarConsultas() {
    ...
    tbody.innerHTML = filtrados.map(c => `
      <tr>
        ...
        <td>
          <button class="btn btn-sm btn-primary" onclick="window.app.verConsulta(${c.id})" title="Ver detalles">
            <i class="bi bi-eye"></i>
          </button>
          <button class="btn btn-sm btn-danger" onclick="window.app.eliminarConsulta(${c.id})" title="Eliminar">
            <i class="bi bi-trash"></i>
          </button>
        </td>
      </tr>
  ...
  actualizarGraficos() {
    ...
    if (topPacientes.length === 0) {
      topDiv.innerHTML = '<p class="text-muted">Sin datos aún</p>';
    } else {
      const maxCount = Math.max(...topPacientes.map(x => x.count));
      topDiv.innerHTML = topPacientes.map(p => `
        <div class="mb-3">
          ...
          <div class="progress" style="height: 10px;">
            <div class="progress-bar bg-success" style="width: ${(p.count / maxCount) * 100}%"></div>
          </div>
        </div>
      `).join('');
    }
  ...
  exportarPDF() {
    ...
    elemento.innerHTML = `
      ...
      <p><strong>Total Pacientes:</strong> ${this.pacientes.length}</p>
      <p><strong>Total Consultas:</strong> ${this.consultas.length}</p>
      <p><strong>Casos Críticos:</strong> ${this.pacientes.filter(p => p.estado === 'critico').length}</p>
    ...
    const opt = {
      margin: 10,
      filename: 'reporte_teleenfermeria.pdf',
  ...
  exportarExcel() {
    let csv = 'Nombre,DNI,Email,Edad,Teléfono,Género,Grupo Sanguíneo,Estado\n';
    this.pacientes.forEach(p => {
      csv += `"${p.nombre}","${p.dni}","${p.email}",${p.edad},"${p.telefono}","${p.genero}","${p.sangre}","${p.estado}"\n`;
    });
    ...
    link.setAttribute('download', 'pacientes_teleenfermeria.csv');
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    window.app = new TeleenfermiaApp();
  });
} else {
  window.app = new TeleenfermiaApp();
}